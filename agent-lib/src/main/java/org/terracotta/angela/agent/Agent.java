...
public class Agent {
  ...
  public static void main(String[] args) {
    ...
    agent.startCluster(Arrays.asList(DIRECT_JOIN.getValue().split(",")), NODE_NAME.getValue(),
  ...
  public void startCluster(Collection<String> peers, String nodeName, int ignitePort) {
    ...
    discoverySpi.setLocalPort(ignitePort);
    discoverySpi.setJoinTimeout(10000);
    discoverySpi.setLocalPortRange(99);
    logger.info("Connecting to peers (size = {}): ", peers.size());
    peers.forEach(peer -> logger.info("- {}", peer));
    discoverySpi.setIpFinder(new TcpDiscoveryVmIpFinder(true).setAddresses(peers));
    cfg.setDiscoverySpi(discoverySpi);
    cfg.setCommunicationSpi(new TcpCommunicationSpi().setLocalPortRange(100).setLocalPort(ignitePort + 10000));
    ...
    controller = new AgentController(ignite, peers, ignitePort);
